#
# Auto-generated by jaxb2ruby https://github.com/sshaw/jaxb2ruby
#
#
# DO NOT MODIFY
# Automatically generated by jaxb2ruby v0.0.1 (https://github.com/sshaw/jaxb2ruby)
#

require "roxml"
require "ddex/element"

require "ddex/ern/v383/musical_work_contributor_role"
require "ddex/ern/v383/party_id"
require "ddex/ern/v383/party_name"
require "ddex/ern/v383/society_affiliation"

module DDEX module ERN module V383  # :nodoc: all

class DDEX::ERN::V383::MusicalWorkContributor < Element
  include ROXML


  xml_name "MusicalWorkContributor"

      xml_accessor :party_names, :as => [DDEX::ERN::V383::PartyName], :from => "PartyName", :required => false
      xml_accessor :party_ids, :as => [DDEX::ERN::V383::PartyId], :from => "PartyId", :required => false
      xml_accessor :musical_work_contributor_roles, :as => [DDEX::ERN::V383::MusicalWorkContributorRole], :from => "MusicalWorkContributorRole", :required => false
      xml_accessor :society_affiliations, :as => [DDEX::ERN::V383::SocietyAffiliation], :from => "SocietyAffiliation", :required => false


  

      xml_accessor :sequence_number, :as => Integer, :from => "@SequenceNumber", :required => false
    
  

end

end end end
