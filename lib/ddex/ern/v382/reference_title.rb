#
# Auto-generated by jaxb2ruby v0.0.1 on 2017-03-14 23:25:47 +0000
# https://github.com/sshaw/jaxb2ruby
#
#
# DO NOT MODIFY
# Automatically generated by jaxb2ruby v0.0.1 (https://github.com/sshaw/jaxb2ruby)
#

require "roxml"
require "ddex/element"

require "ddex/ern/v382/sub_title"
require "ddex/ern/v382/title_text"

module DDEX module ERN module V382  # :nodoc: all

class DDEX::ERN::V382::ReferenceTitle < Element
  include ROXML


  xml_name "ReferenceTitle"

      xml_accessor :title_text, :as => DDEX::ERN::V382::TitleText, :from => "TitleText", :required => true
      xml_accessor :sub_title, :as => DDEX::ERN::V382::SubTitle, :from => "SubTitle", :required => false



      xml_accessor :language_and_script_code, :from => "@LanguageAndScriptCode", :required => false



end

end end end
