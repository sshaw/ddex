#
# Auto-generated by jaxb2ruby v0.0.1 on 2016-03-12 19:07:03 -0800
# https://github.com/sshaw/jaxb2ruby
#
#
# DO NOT MODIFY
# Automatically generated by jaxb2ruby v0.0.1 (https://github.com/sshaw/jaxb2ruby)
#

require "roxml"
require "ddex/element"

require "ddex/ern/v38/all_territory_code"
require "ddex/ern/v38/c_line"
require "ddex/ern/v38/event_date"
require "ddex/ern/v38/genre"
require "ddex/ern/v38/label_name"
require "ddex/ern/v38/name"
require "ddex/ern/v38/p_line"
require "ddex/ern/v38/reference_title"
require "ddex/ern/v38/release_id"
require "ddex/ern/v38/title"

module DDEX module ERN module V38  # :nodoc: all

class DDEX::ERN::V38::CatalogItem < Element
  include ROXML


  xml_name "CatalogItem"

      xml_accessor :territory_codes, :as => [DDEX::ERN::V38::AllTerritoryCode], :from => "TerritoryCode", :required => true
      xml_accessor :release_ids, :as => [DDEX::ERN::V38::ReleaseId], :from => "ReleaseId", :required => true
      xml_accessor :title, :as => DDEX::ERN::V38::Title, :from => "Title", :required => true
      xml_accessor :display_artist_name, :as => DDEX::ERN::V38::Name, :from => "DisplayArtistName", :required => true
      xml_accessor :contributor_names, :as => [DDEX::ERN::V38::Name], :from => "ContributorName", :required => true
      xml_accessor :display_title, :as => DDEX::ERN::V38::ReferenceTitle, :from => "DisplayTitle", :required => true
      xml_accessor :label_names, :as => [DDEX::ERN::V38::LabelName], :from => "LabelName", :required => true
      xml_accessor :genres, :as => [DDEX::ERN::V38::Genre], :from => "Genre", :required => false
      xml_accessor :p_lines, :as => [DDEX::ERN::V38::PLine], :from => "PLine", :required => false
      xml_accessor :c_lines, :as => [DDEX::ERN::V38::CLine], :from => "CLine", :required => false
      xml_accessor :release_date, :as => DDEX::ERN::V38::EventDate, :from => "ReleaseDate", :required => true


  

end

end end end
